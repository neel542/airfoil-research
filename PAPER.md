...
in forward flight, and round a weight-closure loop, so the benchmark says
what the error costs a design and not just how big it is. Every number
carries an error bar built by resampling whole airfoils, and a fitted model
turns any prediction into an expected drag error.
...
does, on 55 percent of points. Match every Princeton polar to its nearest
UIUC one and the two archives differ by 10.9 percent in drag. Read that as
two archives disagreeing rather than as one tunnel's reproducibility limit:
a matched pair is two different hand-built models. The confidence score
warns about drag but not lift, and it tracks XFoil's error more strongly
than the network's. Honest error bars run two to five times wider than naive
ones, and every headline result survives them. One result doesn't. A link
between an airfoil's average confidence and its average error shows up in
the UIUC data and vanishes in the Princeton data, and this paper shows why.
A fitted model reads confidence, Reynolds number, angle and camber and
predicts held-out drag error 14 percent better than a flat guess, in either
tunnel. Running the model on the measured shape of each Princeton wing,
instead of its drawing, removes about a sixth of the drag error. Carried
into a hovering rotor whose blade sits entirely inside the benchmark's
Reynolds range, the drag error arrives divided by three, because two thirds
of hover power is induced. Close the design loop, letting battery mass feed
back into take-off weight, and it doubles again: the amplification factor
from section drag to take-off weight is 0.63 to 0.74, and the 90 percent
interval of the measured error is worth three and a half kilograms on a 15.7
kg aircraft. Hover is the mild case. In forward flight the profile share of
shaft power climbs from 41 to 68 percent, and the same error costs 1.7 times
as much. Finally, a design pipeline uses all of this: a small reward for
staying where NeuralFoil is confident moves a design out of its worst region
for about 2 percent of predicted worst-case L/D.
...
the UIUC tunnel. That makes sense. Fit a network to hundreds of thousands of
XFoil polars and you smooth out XFoil's point-to-point scatter, and some of
that scatter is error. The 3.5 percent
...
it. And hover is the friendly case. Fly the same rotor forward and the
profile share of shaft power climbs from 41 to 68 percent, so the same
section error costs 1.7 times as much at cruise as it does hovering. Uncertainty
...
The design results come with a qualification. Designing against changing
operating conditions is still worth doing. Designing against build error, on
this small ensemble, is not: once the invalid geometries were thrown out the
advantage went with them. The confidence-aware optimizer fares better, and
shows that staying inside NeuralFoil's checked region costs almost nothing in
...
The speed this aircraft would cruise at for endurance is within two metres
per second of the speed where the surrogate's error does the most damage. On
this 15.7 kg aircraft the 90 percent interval of the measured drag error is
worth about three and a half kilograms of take-off mass.
...
checked where this rotor actually runs. No public database has one. A training
set for the surrogate that includes experimental or higher-fidelity slow-speed
data, since the split here shows that more XFoil will not help. And a real
wind-tunnel test of 3D-printed robust and peak-tuned airfoils, with their
built shapes measured, which would turn this validation study into an
